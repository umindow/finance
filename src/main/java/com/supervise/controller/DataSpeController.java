...
import com.supervise.controller.vo.BankCreditQueryVo;
import com.supervise.controller.vo.CommonQueryVo;
...
@RestController
@RequestMapping("/data")
public class DataSpeController {

    private final Logger logger = LoggerFactory.getLogger(DataSpeController.class);
    ...
    public ModelAndView list(@RequestParam(value = "p", required = false) Integer pageNum,
                             @RequestParam(value = "reqJosn", required = false) String reqJosn,
...
//        }
        if (DataType.SUPERVISE_BANK_DATA.getDataLevel() == dataType.intValue()) {
            return new BankCreditDataList().dataList(pageNum, reqJosn, dataType);
        } else if (DataType.SUPERVISE_TRACE_DATA.getDataLevel() == dataType.intValue()) {
            return new TraceDataList().dataList(pageNum, reqJosn, dataType);
        } else if (DataType.SUPERVISE_REPLACE_DATA.getDataLevel() == dataType.intValue()) {
            return new ReplaceDataList().dataList(pageNum, reqJosn, dataType);
        } else if (DataType.SUPERVISE_FEE_DATA.getDataLevel() == dataType.intValue()) {
            return new RefundAndFeeCreditDataList().dataList(pageNum, reqJosn, dataType);
        } else if (DataType.SUPERVISE_BIZ_DATA.getDataLevel() == dataType.intValue()) {
            return new BusinessDataList().dataList(pageNum, reqJosn, dataType);
        } else if (DataType.SUPERVISE_REBACK_DATA.getDataLevel() == dataType.intValue()) {
            return new RepaymentDataList().dataList(pageNum, reqJosn, dataType);
        } else {
            return null;
        }
    }
    ...
    protected class BankCreditDataList implements SimpleDataList {
        @Override
        public ModelAndView dataList(Integer pageNum, String req, Integer dataType) {
            ...
            BankCreditQueryVo bankCreditQueryVo = JSON.parseObject(req, BankCreditQueryVo.class);
            String date =  bankCreditQueryVo.getBatchDate();
            String creditStartDate = bankCreditQueryVo.getCreditStartDate();
            String creditEndDate = bankCreditQueryVo.getCreditEndDate();
            List<BankCreditEntity> bankCreditEntities = bankCreditDao.queryBankCreditByCondition(date,creditStartDate,creditEndDate);
    ...
    protected class TraceDataList implements SimpleDataList {
        ...
        public ModelAndView dataList(Integer pageNum, String req, Integer dataType) {
            ...
            ModelAndView view = new ModelAndView("pages/data/genericDataList", "list", pager);
            CommonQueryVo commonQueryVo = JSON.parseObject(req, CommonQueryVo.class);
            String date =  commonQueryVo.getBatchDate();
            String qclientName = commonQueryVo.getClientName();
            String qprojId = commonQueryVo.getProjId();
            List<RecourseEntity> recourseEntities = recourseDao.queryBankCreditByCondition(date,qprojId);
