...
CREATE TABLE `finance_bank_credit` (
  ...
  `credit_type_id` varchar(40) DEFAULT NULL COMMENT '授信类型：1 综合授信 2 单笔单议',
  `credit_money` decimal(14,2) DEFAULT NULL COMMENT '授信额度',
...
CREATE TABLE `finance_business_data_info` (
  ...
  `client_type` varchar(10) DEFAULT NULL COMMENT '客户类型：1 企业客户  2 个人客户',
  `client_id` varchar(40) DEFAULT NULL COMMENT '客户编码',
  `client_name` varchar(180) DEFAULT NULL COMMENT '客户名称',
  `id_card_type` varchar(10) DEFAULT NULL COMMENT '证件类型：1 军官证 2 护照 3 身份证 4 其它 ',
  ...
  `area_first` varchar(40) DEFAULT NULL COMMENT '所属地区编号（一级）',
  `area_second` varchar(40) DEFAULT NULL COMMENT '所属地区编号（二级）',
  `area_third` varchar(40) DEFAULT NULL COMMENT '所属地区编号（三级）',
  `company_scale` varchar(10) DEFAULT NULL COMMENT '客户规模编码：1 大型企业 2 中型企业 3 小型企业 4 微型企业',
  `is_farming` varchar(10) DEFAULT NULL COMMENT '是否涉农：1 是 2 否',
  `contract_money` decimal(14,2) DEFAULT NULL COMMENT '合同金额',
  ...
  `repay_type_id` varchar(10) DEFAULT NULL COMMENT '还款方式（指客户与银行之间的还款方式）：1、按月等额本金 2 按月等额本息 3 按季等额本金 4 按季等额本息 5 按月付息一次还本 6 到期一次性还本付息 7 不规则 8 其它',
  `pledge_type_id` varchar(10) DEFAULT NULL COMMENT '反担保措施（如存在多项请加，分隔符）：9 抵押 4 质押 5 保证 10 信用 11 其它 ',
  ...
  `proj_status` varchar(40) DEFAULT NULL COMMENT '项目状态：60 保后监管 80 项目代偿 85 项目追偿 90 项目解除',
...
CREATE TABLE `finance_repayment_info` (
  ...
  `principal` decimal(14,2) DEFAULT NULL COMMENT '实际归还本金',
